...
class IncrementalCompileProcessorTest extends Specification {
    ...
    private static IncludeDirectives includes(Collection<File> deps) {
        return new DefaultIncludeDirectives(ImmutableList.copyOf(deps.collect { IncludeWithSimpleExpression.parse('<' + it.name + '>', false) }), ImmutableMap.of(), ImmutableMap.of())
    }
    ...
    def "discovers if unresolved includes have been used"() {
        given:
        def includes = new DefaultIncludeDirectives(ImmutableList.copyOf([new IncludeWithSimpleExpression("MACRO_DEF", false, IncludeType.MACRO)]), ImmutableMap.of(), ImmutableMap.of())
